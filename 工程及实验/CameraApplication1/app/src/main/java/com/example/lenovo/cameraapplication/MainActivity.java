...
import static com.example.lenovo.cameraapplication.PhotoClipperUtil.getImageContentUri;
import static com.example.lenovo.cameraapplication.PhotoClipperUtil.getUriFromFile;
import static com.example.lenovo.cameraapplication.PhotoClipperUtil.saveMyBitmap;
        ...
        switch (requestCode) {
            ...
            case REQUEST_IMAGE_CAPTURE:
                if (resultCode == RESULT_OK) {
                    ...
                    clipperBigPic(this,getUriFromFile(this,headIconFile));
                    //安卓无法识别file://开头的uri
                }
                break;
            case REQUEST_IMAGE_GET:
                if (resultCode == RESULT_OK) {
                    if (data != null) {
                        ...
                        if (originalUri != null) {
                           ...
                           //这里可以用，因为是取相册里的图片，应该封装进了sqlite里面,这里搜索出来的是file开头的
                        }

                        if (filePath != null && filePath.length() > 0) {
//                            Bitmap bitmap = BitmapFactory.decodeFile(filePath,ops);//延时长，可以采用固定大小改善
    ...
    private void initHeadIconFile() {
        headIconFile = getTempFile();
        headFileNameStr = headIconFile.getName().toString();
        headClipFile = getTempFile();
        clipFileNameStr = headClipFile.getName().toString();

        if (!headIconFile.exists()) {
            boolean mkdirs = headIconFile.mkdirs();//如果没有文件夹，那么创建
    ...
    private void choosePhoto() {
        ...
        if (state.equals(Environment.MEDIA_MOUNTED)) {
            ...
            startActivityForResult(openAlbumIntent, REQUEST_IMAGE_GET);
...
//        File pictureFile = headIconFile;
        ...
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.N){
            ...
            pictureUri = FileProvider.getUriForFile(this,
                    "com.example.lenovo.cameraapplication.fileprovider",headIconFile);
        }else{
            intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
    ...
    private void clipperBigPic(Context context, Uri uri) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            intent.setDataAndType(getUriFromFile(this,new File(url)), IMAGE_TYPE);
            //裁剪后图片输出
            Uri outPutUri = getImageContentUri(this,headClipFile);
            //添加上输出的uri
